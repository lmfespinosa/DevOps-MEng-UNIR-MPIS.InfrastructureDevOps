...
resource "null_resource" "clone_infrastructure_repo" {
    ...
    provisioner "local-exec" {
        ...
        command = "git clone https://github.com/lmfespinosa/DevOps-MEng-UNIR-MPIS.Infrastructure && cd DevOps-MEng-UNIR-MPIS.Infrastructure && git branch -a && git checkout master && git fetch --tags && git tag && git branch -a && git remote rm origin && git remote add origin ${var.org_service_url}/${data.azuredevops_project.project.project_name}/_git/MPIS.Infrastructure && git pull --allow-unrelated-histories origin master && git push https://__personalaccesstoken__@dev.azure.com/${var.org_user}/${data.azuredevops_project.project.project_name}/_git/MPIS.Infrastructure master && cd .. && rmdir /s /Q DevOps-MEng-UNIR-MPIS.Infrastructure"
    }
